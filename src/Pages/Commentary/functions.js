...
const storeOriginalValues = (markets) => {
  return markets.reduce((acc, market) => {
    if (market.runner?.[0]) {
      acc[market.marketId] = {
        line: market.runner[0].line || 0,
        predefinedValue: market.predefinedValue || 0
      };
    }
    return acc;
  }, {});
...
const handleCategory31Update = (allMarkets, updatedMarketId, newValue, updateType, isAutomatic = false) => {
  if (!Array.isArray(allMarkets) || allMarkets.length === 0 || !updatedMarketId) return allMarkets;

  // Deep clone the markets
  const updatedMarkets = JSON.parse(JSON.stringify(allMarkets));

  // Get and sort category 31 markets
  const cat31Markets = updatedMarkets
    .filter(market => market?.marketTypeCategoryId === 31)
    .sort((a, b) => a.marketId - b.marketId);

  if (cat31Markets.length === 0) return allMarkets;

  // Find indices for current and main array
  const currentIndex = cat31Markets.findIndex(market => market.marketId === updatedMarketId);
  console.log({ currentIndex });

  const mainArrayIndex = updatedMarkets.findIndex(market => market.marketId === updatedMarketId);

  if (currentIndex === -1 || mainArrayIndex === -1) return allMarkets;

  try {
    // Update the current market
    if (updateType === 'line') {
      const oldLine = updatedMarkets[mainArrayIndex]?.runner?.[0]?.line || 0;
      const difference = +newValue - oldLine;

      updatedMarkets[mainArrayIndex].runner[0].line = +newValue;
      if (!isAutomatic) {
        updatedMarkets[mainArrayIndex].predefinedValue += difference;
      }
    } else if (updateType === 'predefinedValue') {
      updatedMarkets[mainArrayIndex].predefinedValue = +newValue;
    }

    // Update subsequent markets
    for (let i = currentIndex + 1; i < cat31Markets.length; i++) {
      const marketId = cat31Markets[i].marketId;
      const index = updatedMarkets.findIndex(market => market.marketId === marketId);
      const prevMarketId = cat31Markets[i - 1]?.marketId;
      const prevIndex = updatedMarkets.findIndex(market => market.marketId === prevMarketId);

      if (index !== -1 && prevIndex !== -1) {
        const prevLine = updatedMarkets[prevIndex]?.runner?.[0]?.line || 0;
        const currentPredefined = updatedMarkets[index]?.predefinedValue || 0;

        updatedMarkets[index].runner[0].line = prevLine + currentPredefined;
      }
    }

    // Validation step to check for missing or invalid data
    const isValid = updatedMarkets.every(market => market && typeof market.marketTypeCategoryId !== 'undefined');
    if (!isValid) {
      console.warn('Invalid market data detected during update, reverting changes.');
      return allMarkets;
    }

    return updatedMarkets;
  } catch (error) {
    console.error('Error in handleCategory31Update:', error);
    return allMarkets;
  }
};

export { handleCategory31Update, storeOriginalValues };