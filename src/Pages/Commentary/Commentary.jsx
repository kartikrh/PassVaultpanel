...
export const CommentaryScreen = ({
  ...
  const generateBallfromArray = (ballArray = []) => {
    return ballArray?.map((element) => {
      ...
      return (
        <div
          className={` px-3 py - md - 2 py - 1 shadow - sm rounded mx - 1 over-ball-display ${ballColor}`}
        >
          {`${ballValue} ${ballTypeAdd && ballValue ? "| " : ""} ${
            ballTypeAdd || ""
          }`}
        </div>
      );
  ...
  const generateRightSideOvers = () => {
    return Object.keys(overBalls).map((over, index) => (
      <>
        ...
        <div
          className={`ball-by-ball-display ${
            index % 2 !== 0 ? "background-nth " : ""
          } `}
          xs={12}
          md={12}
          lg={12}
    ...
    <React.Fragment>
      <Row>
        ...
        <Col xs={12} md={6} lg={6}>
          ...
          <Row>
            <Col className="current-score-header" xs={6} md={6} lg={6}>
              <span className="current-team-name">
                {teamDetails?.[BATTING_TEAM]?.shortName?.toUpperCase()}&nbsp;
              </span>
            ...
            <Col className="bowling-team-score-header" xs={6} md={6} lg={6}>
              <span className="bowling-team-name">
                {teamDetails?.[BOWLING_TEAM]?.shortName?.toUpperCase()}&nbsp;
              </span>
          ...
          <Row>
            {onPitchPlayers[ON_STRIKE]?.batterOrder >
            onPitchPlayers[NON_STRIKE]?.batterOrder ? (
              <>
                <Col className="non-striker-end" xs={12} md={6} lg={6}>
                  ...
                  <button
                    ...
                    className="change-button text-right"
                  >
                    C
                  </button>
                </Col>
          ...
          <Row className="Bowler-header">
            <Col xs={12} md={12} lg={12}>
              ...
              <span>
                {onPitchPlayers[CURRENT_BOWLER]?.bowlerOver || 0}-
              ...
              <button
                ...
                className=" text-right change-button"
              >
                C
              </button>
            </Col>
          ...
          <Row className={isLoading ? "disable-button" : ""}>
            ...
            >
              <img
                className="button-icon-lg"
                src="icons/leg-by.png"
                alt="Icon"
              />
            </Col>
            <Col
              role="button"
              className="color-out score-button"
              xs={6}
              md={6}
              lg={6}
        ...
        <Col className="over-render" xs={12} md={6} lg={6}>
          ...
          <Row>{generateRightSideOvers()}</Row>
          <img
            role="button"
      ...
      <Row className="mt-5">
        {reactApplicationPhase === "development" ? (
          <iframe
            src={`https://uat.deployed.live/scoreboard2?id=${commentaryId}&color=000`}
            frameborder="0"
          ></iframe>
        ) : (
          <iframe
            src={`https://deployed.live/scoreboard2?id=${commentaryId}&color=000`}
            frameborder="0"
          ></iframe>
        )}
      </Row>
