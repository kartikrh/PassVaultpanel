...
const Index = () => {
  ...
  const getSelectedItemsData = () => {
    ...
    const sourceList = tableSearchedData && tableSearchedData.length > 0
      ? tableSearchedData.map(item => item.templateId)
      : dataIndexList;
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "isActive",
      render: (text, record) => (
      <Tooltip title={"Template"} color={"#e8e8ea"} overlayInnerStyle={{color: '#000'}}>
        <Button
          color={`${text ? "primary" : "danger"}`}
          size="sm"
          className="btn"
  ...
  const handleLoadData = async (password) => {
    setIsLoading(true);
    await axiosInstance
      .post(`/loadPanelData`, {module: [MODULE_TEMPLATE], password})
      .then((response) => {
        fetchData();
