...
const Index = () => {
  ...
  const fetchData = async (latestValueFromTable) => {
    ...
    await axiosInstance
      .post(`/admin/competitionStatisticsType/all`, {
  ...
  const handleSingleCheck = (e) => {
    ...
    if (checekedList.includes(e.competitionStatisticsTypeId)) {
      updateSingleCheck = checekedList.filter((item) => item !== e.competitionStatisticsTypeId);
    } else {
      updateSingleCheck = [...checekedList, e.competitionStatisticsTypeId];
    }
  ...
  const handlePermissions = async (pType, record, cState) => {
    setIsLoading(true);
    await axiosInstance
      .post(`/admin/competitionStatisticsType/save`, {
        competitionStatisticsTypeId: record.competitionStatisticsTypeId,
  ...
  const handleLoadData = async (password) => {
    setIsLoading(true);
    await axiosInstance
      .post(`/loadPanelData`, {module: [MODULE_BANNERS], password})
      .then((response) => {
        fetchData();
  ...
  const handleDelete = async (e) => {
    setIsLoading(true);
    await axiosInstance
      .post(`/admin/competitionStatisticsType/delete`, {
        competitionStatisticsTypeId: checekedList,
      })
      .then((response) => {
        fetchData();
  ...
  const getSelectedItemsData = () => {
    ...
    const sourceList = tableSearchedData && tableSearchedData.length > 0
      ? tableSearchedData.map(item => item.competitionStatisticsTypeId)
      : dataIndexList;
  ...
  const columns = [
      ...
      render: (text, record) => (
        <div className="form-check d-flex align-items-center justify-between">
          <input
            ...
            name="chk_child"
            value="option1"
            checked={checekedList.includes(record.competitionStatisticsTypeId)}
            onChange={() => {
    ...
    checkPermission(permissionObj, pageName, PERMISSION_EDIT) && {
      ...
      render: (text, record) => (
        <i
          ...
          onClick={() => {
            handleEdit(record.competitionStatisticsTypeId);
          }}
        ></i>
      ),
    ...
    // },
    {
      ...
      key: "IsActive",
      render: (text, record) => (
      <Tooltip title={"Competition Statistics"} color={"#e8e8ea"} overlayInnerStyle={{color: '#000'}}>
        <Button
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="ScoreCard" breadcrumbItem="Competition Statistics" />
          ...
          {loadDataModelVisable && 
            <LoadDataModal
              ...
              moduleName={"Competition Statistics"} 
            />}
        </Container>
      </div>
    </React.Fragment>
