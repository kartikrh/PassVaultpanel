...
const Index = () => {
  ...
  const handlePermissions = async (pType, record, cState) => {
    setIsLoading(true);
    await axiosInstance
      .post(`/admin/award/activeInactive`, {
        id: record.id,
  ...
  const handleLoadData = async (password) => {
    setIsLoading(true);
    await axiosInstance
      .post(`/loadPanelData`, {module: [MODULE_AWARDS], password})
      .then((response) => {
        fetchData();
  ...
  const handleEdit = (id) => {
    navigate("/addAward", { state: { awardId: id } });
  };
      ...
      render: (text, record) => (
        <Tooltip title={"Show Award"} color={"#e8e8ea"} overlayInnerStyle={{ color: '#000' }}>
          <Button
            ...
            className="btn"
            onClick={() => {
              handlePermissions("isShowOnSummary", record, record.isShowOnSummary);
            }}
          >
            <i className={`bx ${record.isShowOnSummary ? "bx-check" : "bx-block"}`}></i>
          </Button>
        </Tooltip>
      ),
      style: { width: "2%", textAlign: "center" },
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="ScoreCard" breadcrumbItem="Awards" />
