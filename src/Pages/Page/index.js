...
import { ERROR, MODULE_PAGES, PERMISSION_ADD, PERMISSION_DELETE, PERMISSION_EDIT, PERMISSION_VIEW, SUCCESS, TAB_PAGE } from "../../components/Common/Const";
...
const Index = () => {
  ...
  const fetchData = async (latestValueFromTable) => {
    ...
    await axiosInstance
      ...
      .then((response) => {
        const apiData = response?.result?.sort((a,b)=>a?.pageId - b?.pageId);
        let apiDataIdList = [];
        apiData.forEach(ele => {
  ...
  const handleLoadData = async (password) => {
    setIsLoading(true);
    await axiosInstance
      .post(`/loadPanelData`, {module: [MODULE_PAGES], password})
      .then((response) => {
        fetchData();
  ...
  const getSelectedItemsData = () => {
    ...
    const sourceList = tableSearchedData && tableSearchedData.length > 0
      ? tableSearchedData.map(item => item.pageId)
      : dataIndexList;
